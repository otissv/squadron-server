...
export default class FiscalYear {
  ...
  findById ({ query, args, validation }) {
    ...
    if (Array.isArray(args || query)) {
      getDocuments = r.table(FISCAL_YEARS_TABLE).getAll(...obj.map(i => i.id));
    } else {
      getDocuments = r.table(FISCAL_YEARS_TABLE).get(obj.id);
    }
